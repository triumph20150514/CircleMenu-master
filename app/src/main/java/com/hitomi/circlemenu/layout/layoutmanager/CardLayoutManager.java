package com.hitomi.circlemenu.layout.layoutmanager;
...
public class CardLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        int firstLineSize = mGroupSize / 2 + 1;                      //    2
        int secondLineSize = firstLineSize + mGroupSize / 2;         //    3
        if (isGravityCenter && firstLineSize * itemWidth < getHorizontalSpace()) {
            mGravityOffset = (getHorizontalSpace() - firstLineSize * itemWidth) / 2; //居中显示
            Log.e("Trimph", "offset" + mGravityOffset);
        } else {
            mGravityOffset = 0;
        }
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            Log.e("Trimph", "coefficient" + coefficient);
            Log.e("Trimph", "coefficient------------" + (5 / 2));
            Log.e("Trimph", "offsetHeight" + offsetHeight);
            // 每一组的第一行
